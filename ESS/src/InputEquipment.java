...
public class InputEquipment extends JDialog {
    ...
    String input3=null;
    int dataCheck=0;
	public InputEquipment(String [] sport) {
		setTitle("輸入設備資料");
		//
		...
		//標題
		JLabel label = new JLabel("輸入資料ー設備資料");
		...
		inputButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				inputwindows in=new inputwindows(sport);
				 ...
				 /*in.getTextField();*/
				 if(dataCheck==0){
					 DTM.addRow(new Object[]{input1,sport[input2],input3});
					 dataEquipment.add(input1);
					 dataSport.add(sport[input2]);
					 dataCount.add(input3);
				 }
				 else{
				...
				button_1.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						dataSport.remove(table.getSelectedRow());
		                   dataEquipment.remove(table.getSelectedRow());
		...
		//重新輸入
				...
				removeAll.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						int temp=dataEquipment.size();
						for(int i=0;i<temp;i++){
							dataEquipment.remove(0);
							dataSport.remove(0);
							dataCount.remove(0);
						}
					}
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					WritableSheet sheet = workbook.createSheet("設備資料", 4);
					...
					sheet.addCell(new Label(0,0,"設備名稱",cellFormat));
					sheet.addCell(new Label(1,0,"運動項目",cellFormat));
					sheet.addCell(new Label(2,0,"設備數量",cellFormat));
					int temp=dataEquipment.size();
		            for(int i=1;i<=temp;i++){
		            	sheet.addCell(new Label(  0,i,dataEquipment.get(i-1) ) );
	...
	class inputwindows extends JDialog{
	    ...
	    public inputwindows(String [] sport){
			...
			textField2.setFont(new Font("微軟正黑體", Font.PLAIN,18));
			textField2.setBounds(111, 115, 194, 31);
			...
			JLabel label_1 = new JLabel("設備數量：");
